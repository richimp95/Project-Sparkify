...
def process_song_file(cur, filepath):
    ...
    song_data = [list(row) for row in song_data.itertuples(index=False)][0]
...
def process_log_file(cur, filepath):
    ...
    for index, row in df.iterrows():
        ...
        songplay_data = (str(pd.to_datetime(row.ts, unit='ms')), int(row.userId), row.level, songid, artistid, str(row.sessionId), row.location, row.userAgent)
